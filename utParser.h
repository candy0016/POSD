...
TEST_F(ParserTest, ListAsStruct) {
    ...
    ASSERT_EQ("2", terms->arity());
    ASSERT_EQ(".(1, [])", terms->symbol());
    ASSERT_EQ("1", (*terms->get_args())[0]->symbol());
    ASSERT_EQ("[]", (*terms->get_args())[1]->symbol());
}
...
TEST_F(ParserTest, ListAsStruct2) {
    ...
    Term *terms = parser.createTerm();
    ASSERT_EQ("2", terms->arity());
    ASSERT_EQ(".(2, .(1, []))", terms->symbol());
    ASSERT_EQ("2", (*terms->get_args())[0]->symbol());
    ASSERT_EQ(".(1, [])", (*terms->get_args())[1]->symbol());
}
